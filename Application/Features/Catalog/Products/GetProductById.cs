...
public class GetProductByIdQueryHandler(IProductRepository productRepo, ITenantContext tenant)
    : IRequestHandler<GetProductByIdQuery, ProductDetailDto>
{
    ...
    {
        var p = await productRepo.GetByIdAsync(q.Id, ct)
            ?? throw new NotFoundException("Produto", q.Id);
        ...
        var variants = p.Variants
            .Select(v => new ProductVariantDto(
                ...
                v.StockQuantity,
                v.IsActive,
                v.Attributes.Select(a => new VariantAttributeDto(a.AttributeName, a.AttributeValue)).ToList().AsReadOnly()
            ))
            .ToList()
            .AsReadOnly();
