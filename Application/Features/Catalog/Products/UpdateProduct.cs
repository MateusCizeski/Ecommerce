...
public class UpdateProductCommandHandler(IProductRepository productRepo, IUnitOfWork uow, ITenantContext tenant)
    : IRequestHandler<UpdateProductCommand>
{
    public async Task Handle(UpdateProductCommand cmd, CancellationToken ct)
    {
        ...
        if (product.Slug != cmd.Slug && await productRepo.SlugExistsAsync(tenant.TenantId, cmd.Slug, ct))
            throw new ConflictException($"O slug '{cmd.Slug}' já está em uso.");
